    ...
    <div className="tienda row">
      <div className="container">
        ...
        <div className="col s12 box carrito blue darken-1">
          <h5 className="left">
            <Link to='/catalogo' className='white-text'>Tienda</Link><span className="white-text "> > {this.state.producto.map((producto)=>producto.descripcion)} </span>
          </h5>
          <h5 className="right">
            <Link to={`/producto/${this.state.atras}`} onClick={() => this.setState({refresh : true}).bind(this)} className="btn btn-secondary left disabled={this.navegacionSiguiente(idProducto) === 1) ? true : false}">  <i className="material-icons">keyboard_arrow_left</i> </Link>
            <Link to={`/producto/${this.state.siguiente}`}  onClick={() => this.setState({refresh : true}).bind(this)} className="btn btn-secondary left disabled={(this.navegacionSiguiente(idProducto) >= this.state.listaProductos.length) ? true : false}"><i className="material-icons">keyboard_arrow_right</i> </Link>
          </h5>
        </div>
...
class DetalleProducto extends React.Component{
  ...
  agregarProducto(){
    ...
    const producto = this.props.producto;
    if (cantidad <=0) { //Verificar que la cantidad seleccionada no sea una cantidad válida
    ...
    if(this.state.disponible < cantidad){
      ...
      let disponibles = (Number(this.state.disponible) - Number(cantidad)); //Calcular la disponiblilidad luego de restar la cantidad
      let agregarACarrito = (Number(this.state.contadorCarrito) + Number(cantidad)); //Aumentar el contador de los items en el carrito
      this.setState({disponible : disponibles}); //Actualizar el estado de los productos disponibles en la vista
      this.setState({contadorCarrito : agregarACarrito}); //Actualizar el estado de los productos en el carrito disponibles en la vista
      //Asignar los valores correspondientes al objeto productoCarrito
      ...
      this.props.actualizarDisponible(this.state.productoCarrito, cantidad); //Ejecutar la función actualizar disponible especificando los parámetros productoCarrito, cantidad y
    }
  }
  //------------------------------------------------------------------------------
  ...
  checkCarrito(producto){
    for(let itemCarrito of this.state.listaCarrito){ //Recorrer el arreglo de productos almacenados en el carrito
      if(itemCarrito.id == producto.id){ //Comparar el item del arreglo con los productos en el carrito
        let actualizarDisponible = (Number(this.state.disponible) - Number(itemCarrito.cantidad)); //Crear una variable con la diferencia de la disponible y la cantidad a agregar en el carrito
        this.setState({disponible : actualizarDisponible, contadorCarrito : itemCarrito.cantidad}); //Actualizar el estado disponible y contadorCarrito
      }
    }
  }
  //------------------------------------------------------------------------------
